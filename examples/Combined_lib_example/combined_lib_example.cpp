...
// APP TIMER
const int lorawan_app_interval =
    300000; /**< App payloadTimer interval value in [ms] = 5mins. */
SoftwareTimer
    payloadTimer; /**< payloadTimer to wakeup task and send payload. */
...
static SemaphoreHandle_t semaphore_handle =
    NULL; /**< Semaphore used by events to wake up loop task. */
enum class EVENT_TASK {
  SLEEP,        /**< Use semaphore take to "sleep" in a low power state. */
  SEND_PAYLOAD, /**< Send a sensor reading payload. */
};
static EVENT_TASK current_task =
    EVENT_TASK::SLEEP; /**< Current task of the device, similar to a finite
                          state machine
...
lmh_app_data_t lorawan_payload = {
    payload_buffer, 0, 0, 0, 0}; /**< Struct that passes the payload buffer and
...
// PortSchema.h
static portSchema payload_port =
    PORT1; /**< Frame data port. E.g. port 3: battery voltage + temperature */
...
void fillPayload(void) {
  ...
  // log sensor data
  log(LOG_LEVEL::INFO,
      "Sensor Data: {b: %.2f mV | t: %.2f C | h: %.2f %% | p: %lu Pa | g: %lu "
      "| l: %.5f, %.5f}",
      sensor_data.battery_mv.value, sensor_data.temperature.value,
      sensor_data.humidity.value, sensor_data.pressure.value,
      sensor_data.gas_resist.value, sensor_data.location.latitude,
  ...
  lorawan_payload.buffsize =
      payload_port.encodeSensorDataToPayload(&sensor_data, payload_buffer);

  // log the encoded bytes
  char encoded_payload_bytes[3 * PAYLOAD_BUFFER_SIZE] = {};
  for (int b = 0; b < lorawan_payload.buffsize; b++) {
    snprintf(encoded_payload_bytes, sizeof(encoded_payload_bytes), "%s%02X ",
             encoded_payload_bytes, payload_buffer[b]);
  }
  log(LOG_LEVEL::INFO, "Port: %2.d | Payload: %s", lorawan_payload.port,
