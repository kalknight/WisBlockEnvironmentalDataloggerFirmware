...
bool initPortSensors(portSchema *port) {
    return initSensors(port, false, true);
}

sensorData readPortSensors(portSchema *port) {
    return getSensorData(port);
}
